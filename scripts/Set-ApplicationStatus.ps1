# Set-ApplicationStatus.ps1
# Reads a CSV with "Reference" and "Status" columns and PATCHes
...
if (-not $sampleRow.PSObject.Properties["Status"]) {
    Write-Error "CSV must have a 'Status' column."
    exit 1
}

Write-Host "Found $($rows.Count) row(s)." -ForegroundColor Cyan
...
foreach ($row in $rows) {
    ...
    if (-not $reference -or -not $status) {
        Write-Warning "  SKIP  row with empty Reference or Status (Reference='$reference', Status='$status')"
        continue
    }
    ...
    try {
        ...
        Write-Host "  OK    Reference=$reference  Status=$status" -ForegroundColor Green
        $success++
    }
    catch {
        $status_code = $_.Exception.Response.StatusCode.value__
        Write-Host "  FAIL  Reference=$reference  Status=$status  (HTTP $status_code) $_" -ForegroundColor Red
        $failure++
    }
}
