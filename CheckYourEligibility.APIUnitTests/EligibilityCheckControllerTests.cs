...
namespace CheckYourEligibility.APIUnitTests
{
    public class EligibilityCheckControllerTests : TestBase.TestBase
    {
        ...
        public async Task Given_BulkUploadResults_Exception_Return_500()
        {
            // Arrange
            var request = _fixture.Create<string>();
            _mockCheckService.Setup(x => x.GetBulkCheckResults<IList<CheckEligibilityItem>>(It.IsAny<string>())).Throws<Exception>();
        ...
        public async Task Given_GetItem_Exception_Return_500()
        {
            // Arrange
            var request = _fixture.Create<string>();
            _mockCheckService.Setup(x => x.GetItem<CheckEligibilityItem>(It.IsAny<string>())).Throws<Exception>();
