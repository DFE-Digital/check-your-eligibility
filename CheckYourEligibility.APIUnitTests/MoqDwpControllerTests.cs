...
namespace CheckYourEligibility.APIUnitTests
{
    public class MoqDwpControllerTests : TestBase.TestBase
    {
        ...
        {
            // Arrange
            IMatchCitizenUseCase matchCitizenUseCase = null;
            IGetCitizenClaimsUseCase getCitizenClaimsUseCase = null;

            // Act
            Action act = () => new MoqDWPController(_mockLogger, matchCitizenUseCase, getCitizenClaimsUseCase);

            // Assert
            act.Should().ThrowExactly<ArgumentNullException>().And.Message.Should().EndWithEquivalentOf("Value cannot be null. (Parameter 'matchCitizenUseCase')");
        }
